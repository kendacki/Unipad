import { NextResponse } from "next/server";
import { AuthError, requireAuth } from "@/lib/auth/requireAuth";
import {
  ListingHttpError,
  listListingAllowlist,
  upsertListingAllowlist,
} from "@/lib/listingStore";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

type Ctx = { params: Promise<{ id: string }> };

function handleErr(err: unknown) {
  if (err instanceof AuthError) {
    return NextResponse.json({ error: err.message, code: err.code }, { status: 401 });
  }
  if (err instanceof ListingHttpError) {
    return NextResponse.json(
      { error: err.message, code: err.code },
      { status: err.status },
    );
  }
  return NextResponse.json(
    { error: (err as Error).message || "Failed", code: "UPAD_UNKNOWN" },
    { status: 500 },
  );
}

export async function GET(request: Request, ctx: Ctx) {
  try {
    const session = await requireAuth(request.headers.get("Authorization"));
    const { id } = await ctx.params;
    const phaseId = new URL(request.url).searchParams.get("phaseId") ?? undefined;
    const entries = await listListingAllowlist(session.principal, id, phaseId);
    return NextResponse.json({ entries });
  } catch (err) {
    return handleErr(err);
  }
}

export async function POST(request: Request, ctx: Ctx) {
  try {
    const session = await requireAuth(request.headers.get("Authorization"));
    const { id } = await ctx.params;
    const body = (await request.json()) as {
      phaseId?: string;
      entries?: Array<{ walletPrincipal: string; maxMints?: number }>;
    };
    if (!body.phaseId || !Array.isArray(body.entries) || !body.entries.length) {
      return NextResponse.json(
        { error: "phaseId and entries required", code: "UPAD_VALIDATION" },
        { status: 400 },
      );
    }
    if (body.entries.length > 5000) {
      return NextResponse.json(
        { error: "Allowlist too large (max 5000)", code: "UPAD_VALIDATION" },
        { status: 400 },
      );
    }
    const entries = await upsertListingAllowlist(
      session.principal,
      id,
      body.phaseId,
      body.entries,
    );
    return NextResponse.json({ entries });
  } catch (err) {
    return handleErr(err);
  }
}
